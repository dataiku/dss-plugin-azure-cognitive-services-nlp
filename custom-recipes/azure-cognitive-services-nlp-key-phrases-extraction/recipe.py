...
keyphrases_column = generate_unique('keyphrases', input_columns_names)
...
def detect_keyphrases(text_list):
...
for batch in run_by_batch(detect_keyphrases, input_df, text_column, batch_size=BATCH_SIZE, parallelism=PARALLELISM):
    ...
    for i, raw_result in enumerate(response.get("documents")):
        ...
        input_df.set_value(j, keyphrases_column, json.dumps(output['keyphrases']))
