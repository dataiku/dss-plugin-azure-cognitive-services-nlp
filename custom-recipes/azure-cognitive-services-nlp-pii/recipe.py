...
from azure_nlp_api_formatting import PIIExtractionAPIFormatter
...
def call_api_pii_extraction(batch: List[Dict], text_column: AnyStr, text_language: AnyStr) -> List[Dict]:
    document_list = {
        "documents": [
            {"id": str(index), "text": str(row.get(text_column, "")).strip(), "language": text_language}
            for index, row in enumerate(batch)
        ]
    }
    responses = client.recognize_entities_pii(document_list)
    return responses


df = api_parallelizer(
    input_df=input_df,
    api_call_function=call_api_pii_extraction,
...
api_formatter = PIIExtractionAPIFormatter(
    input_df=input_df,
    text_column=text_column,
